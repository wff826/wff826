<!-- 💖 Pink Theme GitHub Profile -->
<h1 align="center">Hi 👋 I'm Minjeong Kim (wff826)</h1>
<h3 align="center">🌸 Yonsei Univ. | AI Dev | Hackathon Enthusiast 🌸</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com/?lines=💻+AI+Engineer+%7C+🎨+Frontend+Dev+%7C+🧠+Product+Thinker&center=true&width=500&height=45">
</p>

---

### 👩‍🎓 About Me
- 🎓 **Undergraduate @ Yonsei University** Majoring in **Civil & Environmental Engineering**
- 💡 Passionate about **AI-based service development** and **UX-focused prototyping**
- 🛠️ Building a [JWT + Kakao Login System](https://github.com/wff826/skyst)
- 🏃‍♀️ Hackathon enthusiast: I turn ideas into working products — fast!
- 💬 Always open to meaningful collaboration — let’s build something cool.

---

### 🎯 Featured Projects & Experience

#### ✨ Personalized Route Recommendation System  
A smart mobility project based on real-world context and user intent
- 🗺️ **NAVER Maps API**, 📡 **Public Data API**
- 🎥 **YOLO traffic analysis**, 🤖 **ChatGPT for natural language filtering**
- 🧩 Connected **real-time user mobility** to intelligent path optimization

---

#### 🚌 2023 Channel Talk Hackathon – *Commute Alert System*
- 🚏 Personalized commuting assistant using NAVER Maps + public transport APIs
- 🧑‍💼 Focused on optimizing busy office worker routes
- 🏆 **Grand Prize Winner**

---

#### 📝 2024 Yonsei–Korea Hackathon – *Mandarat To-Do Web App*
- 🎯 Designed a goal-setting tool based on the **Mandarat method**
- 🧠 Stood out with **creative UX** and strong insight capture
- 🥇 **1st Place Award**

---

### 💻 Tech Stack

<p align="center">
  <img src="https://img.shields.io/badge/-React-FFB6C1?style=for-the-badge&logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/-JavaScript-FFC0CB?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/-Python-FF69B4?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/-Spring Boot-FDB9C8?style=for-the-badge&logo=springboot&logoColor=white" />
  <img src="https://img.shields.io/badge/-Django-F88379?style=for-the-badge&logo=django&logoColor=white" />
  <img src="https://img.shields.io/badge/-Git-FD5DA8?style=for-the-badge&logo=git&logoColor=white" />
</p>

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=wff826&show_icons=true&theme=radical" />
</p>

---

### ✉️ Contact Me
- 📮 Email: `m2njeong826@yonsei.ac.kr`
- 🌐 Portfolio & Blog: *Coming soon!*

